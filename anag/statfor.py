...
import anag.statfor_wdr as wdr
...
FRAME_TITLE = "Status Fornitori"


class StatForPanel(ga.AnagPanel):
    def __init__(self, *args, **kwargs):
        ga.AnagPanel.__init__(self, *args, **kwargs)
        self.SetDbSetup( bt.tabelle[ bt.TABSETUP_TABLE_STATFOR] )
        self.db_report = "Status Fornitori"
    
    def InitAnagCard(self, parent):
        p = wx.Panel( parent, -1)
        wdr.StatForCardFunc( p, True )
        return p
...
class StatForFrame(ga._AnagFrame):
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.LoadAnagPanel(StatForPanel(self, -1))


# ------------------------------------------------------------------------------


class StatForDialog(ga._AnagDialog):
    """
